...
class OciAdapter extends Connection {
    ...
    protected function __construct($info) {
        $tns = "(DESCRIPTION =
                                            (ADDRESS_LIST =
                                              (ADDRESS = (COMMUNITY = tcp.world)(PROTOCOL = TCP)(HOST = " . $info->host . ")(PORT = 1521))
                                            )
                                            (CONNECT_DATA =
                                              (SID = " . $info->db . ")
                                            )
        ...
        try {
            $this->connection = new PDO("oci:dbname=" . $tns . ';charset=UTF8', $info->user, $info->pass, static::$PDO_OPTIONS);
